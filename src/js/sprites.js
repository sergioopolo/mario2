...
function spriteUnravel(colors) {
  var myfix = false;
  if(colors.indexOf('myfix') !== -1){
    colors = colors.replace("myfix", ""); 
    myfix = true;
  }
    ...
    switch(colors[loc]) {
      ...
      case 'p':
        ...
        if(colors[++loc] == '[') {
          ...
          loc = nixloc + 1;
          digitsize = (Object.keys(paletteref).length > 10) ? 2 : 1;
        }
...
function spriteGetArray(colors) {
  ...
  /*if(spriteGetArrayTest) {  
    console.log(split) ; 
    spriteGetArrayTest = false;
  }*/
...
function expandObtainedSprite(sprite, thing, width, height, norefill) {
  ...
  var notScale = ['Player', 'Mushroom', 'FireFlower', 'Mushroom1Up', 'Goomba', 'Koopa', 'DeadGoomba', 'Pirhana', 'Bank', 'Bowser', 'Viol'];
  if(PlayerMyfix && (-1 !== notScale.indexOf(thing.title))){
    parsed = sprite;
  }
  else {
