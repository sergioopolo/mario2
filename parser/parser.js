  ...
  img.onload = function() { console.log('loaded');
    ...
    setTimeout(function() {
      ...
      var res = document.getElementById("fin"),
          canN = document.createElement("canvas"),
          ctx = canN.getContext("2d");

      ctx.putImageData(data, 0, 0);
      res.appendChild(canN);
    }, 650);
  }
...
function makedigit(num, digitsize, newpalette) {
  //console.log('fill for ' + num, digitsize - String(newpalette[num]).length);
  //console.log(newpalette);
  return fillChars(max(0, digitsize - String(newpalette[num]).length), "0") + newpalette[num];
}
